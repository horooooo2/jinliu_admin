<template>
  <div class="pay-behalf-report-page">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2 class="page-title">代付报表</h2>
    </div>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="代理佣金总额" width="150" align="right">
          <template #default="{ row }">
        ...
        <el-table-column label="冲正总额" width="150" align="right">
          <template #default="{ row }">
            <span style="color: #F56C6C;">
              {{ row.reversalMoney?.toFixed(2) }}
            </span>
          </template>
        </el-table-column>

        <el-table-column prop="reversalCount" label="冲正笔数" width="120" align="right" />

        <el-table-column label="冲正手续费" width="150" align="right">
          <template #default="{ row }">
            <span style="color: #F56C6C;">
              {{ row.reversalFee?.toFixed(2) }}
            </span>
          </template>
        </el-table-column>
...
const calculateSummary = () => {
  ...
  summaryData.totalCommission = tableData.value.reduce((sum, item) => sum + (item.commission || 0), 0)
  ...
  summaryData.totalReversalMoney = tableData.value.reduce((sum, item) => sum + (item.reversalMoney || 0), 0)
  summaryData.totalReversalCount = tableData.value.reduce((sum, item) => sum + (item.reversalCount || 0), 0)
  summaryData.totalReversalFee = tableData.value.reduce((sum, item) => sum + (item.reversalFee || 0), 0)
}
  ...
  columns.forEach((column: any, index: number) => {
    ...
    switch (column.property) {
      ...
      case 'fee':
        ...
        break
      case 'commission':
        sums[index] = summaryData.totalCommission.toFixed(2)
        break
      case 'successCount':
      ...
      case 'submitCount':
        ...
        break
      case 'reversalMoney':
        sums[index] = summaryData.totalReversalMoney.toFixed(2)
        break
      case 'reversalCount':
        sums[index] = summaryData.totalReversalCount.toString()
        break
      case 'reversalFee':
        sums[index] = summaryData.totalReversalFee.toFixed(2)
        break
      default:
        sums[index] = ''
    }
...
const handleExport = async () => {
  try {
    ...
    if (activeTab.value === 'daily') {
      exportUrl = '/pay-report/payout-order-daily/export'
      fileNamePrefix = 'pay_behalf_daily_report'
    } else if (activeTab.value === 'monthly') {
      exportUrl = '/pay-report/payout-order-daily/export-month'
      fileNamePrefix = 'pay_behalf_monthly_report'
    }
