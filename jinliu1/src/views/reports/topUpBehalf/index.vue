<template>
  <div class="topup-behalf-report-page">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2 class="page-title">代充报表</h2>
    </div>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="差价利润" width="150" align="right">
          <template #default="{ row }">
            <span style="color: #409EFF; font-weight: 600;">
              {{ row.priceDiffProfit?.toFixed(2) }}
            </span>
          </template>
        </el-table-column>

        <el-table-column label="手续费" width="150" align="right">
          <template #default="{ row }">
...
const calculateSummary = () => {
  ...
  summaryData.totalPriceDiffProfit = tableData.value.reduce((sum, item) => sum + (item.priceDiffProfit || 0), 0)
  ...
  columns.forEach((column: any, index: number) => {
    ...
    switch (column.property) {
      case 'money':
        ...
        break
      case 'priceDiffProfit':
        sums[index] = summaryData.totalPriceDiffProfit.toFixed(2)
        break
      case 'fee':
...
const handleExport = async () => {
  try {
    ...
    if (activeTab.value === 'daily') {
      exportUrl = '/pay-report/payout-topup-order-daily/export'
      fileNamePrefix = 'topup_behalf_daily_report'
    } else if (activeTab.value === 'monthly') {
      exportUrl = '/pay-report/payout-topup-order-daily/export-month'
      fileNamePrefix = 'topup_behalf_monthly_report'
    }
