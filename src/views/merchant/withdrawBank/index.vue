<template>
  <div class="withdraw-bank-page">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2 class="page-title">提现银行管理</h2>
    </div>
...
import {
  getMerchantWithdrawalBankList,
  addMerchantWithdrawalBank,
  editMerchantWithdrawalBank,
  editMerchantWithdrawalBankName,
  editMerchantWithdrawalBankSort,
  deleteMerchantWithdrawalBank,
  type MerchantPayoutBankParams,
  type MerchantPayoutBankAddParams,
} from '@/api/merchant/withdrawBank'
...
const fetchBankList = async () => {
  try {
    ...
    const res = await getMerchantWithdrawalBankList(params)
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        const apiFunc = isEdit.value ? editMerchantWithdrawalBank : addMerchantWithdrawalBank
  ...
  await editNameFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        const res = await editMerchantWithdrawalBankName({
          id: editNameForm.id,
  ...
  await editSortFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        const res = await editMerchantWithdrawalBankSort({
          id: editSortForm.id,
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(
    ...
    try {
      const res = await deleteMerchantWithdrawalBank([{ id: row.id }])
...
const handleBatchDelete = () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请选择要删除的银行')
    return
  }

  ElMessageBox.confirm(
    `确定要删除选中的 ${selectedIds.value.length} 个银行吗？此操作不可恢复！`,
    '警告',
    {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
    ...
    try {
      const deleteParams = selectedIds.value.map(id => ({ id }))
      const res = await deleteMerchantWithdrawalBank(deleteParams)
